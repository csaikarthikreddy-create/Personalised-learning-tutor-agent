...
class FeatureEngineer:
    ...
    def _process_tick(self, tick_data: Dict) -> Optional[Dict]:
        ...
        volatility = self._compute_volatility(window_returns, window_size=min(60, len(window_returns)))
            ...
            while self.running:
                ...
                try:
                    ...
                    if features:
                        ...
                        if len(self.feature_buffer) >= self.config['features']['parquet_batch_size']:
                            self._save_to_parquet()
                            last_save_time = time.time()
