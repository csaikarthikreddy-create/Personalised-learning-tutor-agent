...
def time_based_split(df: pd.DataFrame, config: Dict) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    ...
    if 'label' in df.columns:
        ...
        logger.info(f"Overall dataset: {total_positives} positives ({overall_positive_rate*100:.2f}%)")
        ...
        target_test_positives = total_positives - target_train_positives - target_val_positives  # Remaining
        
        logger.info(f"Target positives (proportional) - Train: {target_train_positives}, Val: {target_val_positives}, Test: {target_test_positives}")
        ...
        if train_pos_count < target_train_positives:
            ...
            for idx in reversed(positive_indices):
                if idx < train_end and train_pos_count > target_train_positives and train_end > min_train_size:
                    train_pos_count -= 1
                    train_end = idx
                    if train_pos_count <= target_train_positives:
                        break
        ...
        logger.info(f"Target vs Actual - Train: {target_train_positives} vs {train_pos} (diff: {train_pos - target_train_positives})")
        logger.info(f"Target vs Actual - Val: {target_val_positives} vs {val_pos} (diff: {val_pos - target_val_positives})")
        logger.info(f"Target vs Actual - Test: {target_test_positives} vs {test_pos} (diff: {test_pos - target_test_positives})")
    ...
    if 'label' in df.columns:
        ...
        logger.info(f"  Train - Spikes: {train_pos}, Normal: {(train_df['label']==0).sum()}, Rate: {train_df['label'].mean()*100:.2f}%")
        logger.info(f"  Validation - Spikes: {val_pos}, Normal: {(val_df['label']==0).sum()}, Rate: {val_df['label'].mean()*100:.2f}%")
        logger.info(f"  Test - Spikes: {test_pos}, Normal: {(test_df['label']==0).sum()}, Rate: {test_df['label'].mean()*100:.2f}%")
        ...
        if train_pos == 0 or val_pos == 0 or test_pos == 0:
            ...
            logger.warning("   This may affect model training. Consider adjusting threshold_percentile in config.")
...
def train_baseline_model(X_train, y_train, X_val, y_val, config: Dict, run_name: str = "baseline"):
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        logger.info(f"Baseline model - Val PR-AUC: {val_metrics.get('pr_auc', 0):.4f}, Val F1: {val_metrics.get('f1', 0):.4f}")
        
        return baseline, val_metrics


def train_ml_model(X_train, y_train, X_val, y_val, config: Dict, model_type: str = "logistic", run_name: str = "ml_model"):
    ...
    with mlflow.start_run(run_name=run_name):
                ...
                for solver in ['lbfgs', 'liblinear']:
                    try:
                        ...
                        if y_val.sum() > 0:
                            ...
                            logger.debug(f"  C={c_val}, solver={solver}: No positives in validation, using training set")
                            y_proba_train_temp = temp_model.predict_proba(X_train_scaled)[:, 1]
                            pr_auc_temp = average_precision_score(y_train, y_proba_train_temp) if y_train.sum() > 0 else 0.0
                        ...
                        if pr_auc_temp > best_pr_auc:
                            ...
                            best_c = c_val
                            best_scaler = scaler
                            logger.info(f"  C={c_val}, solver={solver}: PR-AUC={pr_auc_temp:.4f} (new best)")
        ...
        if y_val.sum() == 0:
            logger.warning("Validation set has no positive samples! Using training set for threshold optimization.")
        ...
        if precision_mask.any():
            ...
            threshold_precision = thresholds[best_precision_idx] if best_precision_idx < len(thresholds) else 0.5
        else:
        ...
        if model_type == "logistic":
            mlflow.log_metric("val_precision_at_precision_threshold", val_metrics_precision['precision'])
        ...
        try:
            ...
            logger.warning(f"Could not log model artifact to MLflow: {e}. Model saved to {model_path}")
        ...
        logger.info(f"{model_type} model - Val PR-AUC: {val_metrics.get('pr_auc', 0):.4f}, Val F1: {val_metrics.get('f1', 0):.4f}")
...
def main():
    ...
    try:
        ...
        ml_model, scaler, ml_metrics = train_ml_model(
            ...
            model_type=args.model_type, run_name=f"ml_{args.model_type}"
        )
        ...
        logger.info("=" * 60)
        logger.info(f"Baseline - Val PR-AUC: {baseline_metrics.get('pr_auc', 0):.4f}, Val F1: {baseline_metrics.get('f1', 0):.4f}")
        logger.info(f"ML Model - Val PR-AUC: {ml_metrics.get('pr_auc', 0):.4f}, Val F1: {ml_metrics.get('f1', 0):.4f}")
        logger.info("=" * 60)
