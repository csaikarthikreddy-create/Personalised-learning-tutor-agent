...
def generate_report(test_df: pd.DataFrame, mlflow_results: Dict, output_dir: Path):
    ...
    if 'baseline' in mlflow_results:
        print(f"  Baseline - PR-AUC: {mlflow_results['baseline']['metrics']['pr_auc']:.4f}, F1: {mlflow_results['baseline']['metrics']['f1']:.4f}")
    if 'ml_model' in mlflow_results:
        print(f"  ML Model - PR-AUC: {mlflow_results['ml_model']['metrics']['pr_auc']:.4f}, F1: {mlflow_results['ml_model']['metrics']['f1']:.4f}")
