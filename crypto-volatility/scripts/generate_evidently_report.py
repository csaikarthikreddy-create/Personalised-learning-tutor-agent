...
def save_report_html(report: Report, output_path: Path) -> None:
    ...
    if not saved:
        logger.warning(f"Could not find standard save method for Report. Available methods: {[m for m in dir(report) if not m.startswith('_')][:10]}")
    ...
    for item in report.items():
        ...
        if hasattr(item, 'columns'):
            try:
                cols = item.columns
                if cols is not None:
                    metric_info['columns'] = list(cols) if hasattr(cols, '__iter__') and not isinstance(cols, str) else []
                else:
...
def main():
    ...
    try:
        ...
        if args.compare_train_test:
            ...
            if len(df) < 100:
                logger.warning(f"Only {len(df)} rows available. Need at least 100 for meaningful report.")
                return
